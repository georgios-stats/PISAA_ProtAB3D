...
/*double logsumexp_weighted(double *x,double *y,int n){
	...
	cc = x[1] ;
	for (i=1; i<=n; i++) if (x[i]>cc) cc = x[i] ;

	logsum = 0.0 ;
	for (i=1; i<=n; i++) logsum += exp(x[i]-cc)*y[i] ;
	logsum = cc +log(logsum) ;
...
int main(int argc, char *argv[]){
	...
	grid_low = -165.0 ;
	grid_high = 0.0 ;
	grid_size = (int) ( grid_high-grid_low)/0.1 +1 ;
	...
	snprintf(file_name, sizeof file_name, "./output_files/hist-n=%d-s=%g-r=%d",
			N_population, gain_pow, ID);
    ins_hist = fopen( file_name , "w" ) ;

    if ( N_sample>0 ) {
    snprintf(file_name, sizeof file_name, "./output_files/fz_best_trace-n=%d-s=%g-r=%d",
			N_population, gain_pow, ID);
    ins_fz_best_trace = fopen( file_name , "w" ) ;
    }

    snprintf(file_name, sizeof file_name, "./output_files/fz_best-n=%d-s=%g-r=%d",
			N_population, gain_pow, ID);
	...
	for( iter=1 ; iter<=N_iteration ; iter++ ) {
		...
		case 3 :
			...
			if (scl_mh3 > 10.0) scl_mh3 = 1.0 ;
				scl_mh3*=exp(pow(5.0/n_mh3,0.6)*(accpr-0.2)) ;
			break ;
		case 4 :
			/*sample*/
